...
\ifthenelse{\boolean{nopics}}
             {\quote{\textsl{There are nine circles in a row, from the left cicles one, three and six are red, the rest are blue.}}}
             {
               \begin{center}
  \begin{tikzpicture}
...
%
\ifthenelse{\boolean{nopics}}
           {\quote{\textsl{There are two rows of nine circles, one above the other, in the top row circles one and six are red, the rest blue; in the lower row three is also red; there is an arrow from the top row to the bottom one.}}}
             {
             \begin{center}
\begin{tikzpicture}
...
\end{equation}
The idea is that following this online rule will bring you to values
of the online rule leading to the pattern introduced in the formula
for creating the correct minima: Eqn.~\ref{eq:hop}. The $\alpha$'s are an ofset value which allow the network to reacha stable equilibrium since $\delta w_{ij}$ should stopping changing, on average, if the average fraction of iterations where $x_i$ or $x_j$ is plus one is equal to $\alpha$.
